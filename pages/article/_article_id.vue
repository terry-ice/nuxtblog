      ...
      <div class="metas" v-if="!fetching && article.title">
        ...
        <p class="item">
          ...
          <span class="site-url" @click="copyArticleUrl">
                <span>http://www.592php.com/article/{{ article.id }}</span>
          </span>
        </p>
        <div class="item">
          <span>版权声明：</span>
    ...
    methods: {
      ...
      copyArticleUrl() {
        if (this.article.title) {
          // console.log('要复制了', `http://www.592php.com/article/${this.article.id}`, this.$root.$copyToClipboard)
          this.$root.$copyToClipboard(`http://www.592php.com/article/${this.article.id}`)
        }
      },
      buildThumb(thumb) {
