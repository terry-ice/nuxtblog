  ...
  <div id="share">
    <div class="share-box" :class="{ mobile: mobileLayout }">
      ...
      <a title="豆瓣" 
         ...
         @click.prevent="shareWindow(`http://www.douban.com/recommend/?url=${url}&title=${title()}`)">
      ...
      <a title="印象笔记" 
         ...
         @click.prevent="shareWindow(`http://www.evernote.com/clip.action?url=${url}&title=${title()}`)">
      ...
      <a title="facebook" 
         ...
         @click.prevent="shareWindow(`http://www.facebook.com/sharer/sharer.php?u=${url}`)">
...
<script>
  export default {
    ...
    computed: {
      url() {
        return `https://592php.com${this.$route.fullPath}`
      },
      mobileLayout() {
